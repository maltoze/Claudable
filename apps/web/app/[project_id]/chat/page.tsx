    ...
    claude: [
        {
            ...
            aliases: ["claude-sonnet-4-20250514", "sonnet-4"],
        },
        {
            id: "claude-opus-4.1",
            name: "Claude Opus 4.1",
            aliases: ["claude-opus-4-1-20250805", "opus-4.1"],
        },
        {
            id: "claude-opus-4",
            name: "Claude Opus 4",
            aliases: ["claude-opus-4-20250514", "opus-4"],
        },
        {
            id: "claude-haiku-3.5",
            name: "Claude Haiku 3.5",
            aliases: ["claude-3-5-haiku-20241022", "haiku-3.5"],
        },
    ],
    cursor: [
        { id: "gpt-5", name: "GPT-5" },
...
const MODEL_NAME_OVERRIDES: Record<string, string> = Object.values(
    MODEL_FALLBACKS
)
    .flat()
    .reduce((acc, model) => {
...
const hexToFilter = (hex: string): string => {
    ...
    const filters: { [key: string]: string } = {
        "#DE7356":
            "brightness(0) saturate(100%) invert(52%) sepia(73%) saturate(562%) hue-rotate(336deg) brightness(95%) contrast(91%)", // Orange for Claude
        "#6B7280":
            "brightness(0) saturate(100%) invert(47%) sepia(7%) saturate(625%) hue-rotate(174deg) brightness(92%) contrast(82%)", // Gray for Cursor
        "#A855F7":
            "brightness(0) saturate(100%) invert(48%) sepia(79%) saturate(1532%) hue-rotate(256deg) brightness(95%) contrast(101%)", // Purple for Qwen
        "#4285F4":
            "brightness(0) saturate(100%) invert(40%) sepia(97%) saturate(1449%) hue-rotate(198deg) brightness(97%) contrast(101%)", // Blue for Gemini
...
const buildModelOptions = (
    statuses: Record<string, CliStatusSnapshot>
): ModelOption[] => {
                    ...
                    <div key={fullPath}>
                        ...
                        >
                            ...
                            <div className="w-4 flex items-center justify-center mr-0.5">
                                {entry.type === "dir" &&
                                    (isExpanded ? (
                                        <span className="w-2.5 h-2.5 text-gray-600 dark:text-[#8b8b8b] flex items-center justify-center">
                            ...
                            <span className="w-4 h-4 flex items-center justify-center mr-1.5">
                                {entry.type === "dir" ? (
                                    isExpanded ? (
                                        <span className="text-amber-600 dark:text-[#c09553] w-4 h-4 flex items-center justify-center">
...
export default function ChatPage({ params }: Params) {
    ...
    const { hasActiveRequests, createRequest, startRequest, completeRequest } =
    ...
    const [folderContents, setFolderContents] = useState<Map<string, Entry[]>>(
        new Map()
    );
    ...
    const [uploadedImages, setUploadedImages] = useState<
        { name: string; url: string; base64: string }[]
    >([]);
    ...
    const [projectStatus, setProjectStatus] =
        useState<ProjectStatus>("initializing");
    ...
    const [vercelConnected, setVercelConnected] = useState<boolean | null>(
        null
    );
    const [publishedUrl, setPublishedUrl] = useState<string | null>(null);
    const [deploymentId, setDeploymentId] = useState<string | null>(null);
    const [deploymentStatus, setDeploymentStatus] = useState<
        "idle" | "deploying" | "ready" | "error"
    >("idle");
    const deployPollRef = useRef<NodeJS.Timeout | null>(null);
    const [isStartingPreview, setIsStartingPreview] = useState(false);
    const [previewInitializationMessage, setPreviewInitializationMessage] =
        useState("Starting development server...");
    const [cliStatuses, setCliStatuses] = useState<
        Record<string, CliStatusSnapshot>
    >({});
    ...
    const cliOptions = useMemo(
        () =>
            CLI_ORDER.map((cli) => ({
                ...
                available: Boolean(
                    cliStatuses[cli]?.available && cliStatuses[cli]?.configured
                ),
            })),
        [cliStatuses]
    );
    ...
    const loadCliStatuses = useCallback(async () => {
        ...
        try {
            const response = await fetch(
                `${API_BASE}/api/chat/${projectId}/cli-status`
            );
    ...
    const { setShow429Modal } = useInsufficientBalance();

    const fetchUserInfo = useCallback(async () => {
        try {
            const token = await (
                window as any
            ).electronAPI?.safeStorage.getItem("token");
            const resp = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/work-user/${process.env.NEXT_PUBLIC_INSTANCE_ID}`,
                {
                    headers: {
    ...
    const handleModelChange = useCallback(
        async (
            option: ModelOption,
            opts?: { skipCliUpdate?: boolean; overrideCli?: string }
        ) => {
            ...
            try {
                if (!skipCliUpdate && targetCli !== previousCli) {
                    const cliResponse = await fetch(
                        `${API_BASE}/api/chat/${projectId}/cli-preference`,
                        {
                            method: "POST",
                    ...
                    if (!cliResponse.ok) {
                        ...
                        throw new Error(
                            errorText || "Failed to update CLI preference"
                        );
                    }
                }

                const modelResponse = await fetch(
                    `${API_BASE}/api/chat/${projectId}/model-preference`,
                    {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                ...
                if (!modelResponse.ok) {
                    ...
                    throw new Error(
                        errorText || "Failed to update model preference"
                    );
                }
                ...
                const resolvedModel = normalizeModelId(
                    targetCli,
                    data?.selected_model || newModelId
                );
                ...
                try {
                    ...
                    console.warn(
                        "Failed to record model switch message:",
                        messageError
                    );
                }
    ...
    const handleCliChange = useCallback(
        async (cliId: string) => {
            ...
            try {
                ...
                const candidateModels = modelOptions.filter(
                    (option) => option.cli === cliId
                );
                const fallbackOption =
                    candidateModels.find(
                        (option) =>
                            option.id === previousModel && option.available
                    ) ||
    ...
    useEffect(() => {
        ...
        const hasSelected = modelOptions.some(
            (option) =>
                option.cli === preferredCli && option.id === selectedModel
        );
        if (!hasSelected) {
            const fallbackOption =
                modelOptions.find(
                    (option) => option.cli === preferredCli && option.available
                ) ||
    ...
    const loadDeployStatus = useCallback(async () => {
        try {
            ...
            const response = await fetch(
                `${API_BASE}/api/projects/${projectId}/services`
            );
            if (response.ok) {
                const connections = await response.json();
                const githubConnection = connections.find(
                    (conn: any) => conn.provider === "github"
                );
                const vercelConnection = connections.find(
                    (conn: any) => conn.provider === "vercel"
                );
    ...
    const checkCurrentDeployment = useCallback(async () => {
        try {
            const response = await fetch(
                `${API_BASE}/api/projects/${projectId}/vercel/deployment/current`
            );
            if (response.ok) {
                const data = await response.json();
                if (data.has_deployment) {
                    ...
                    console.log(
                        "🔍 Resuming deployment monitoring:",
                        data.deployment_id
                    );
                }
            }
        ...
        (depId: string) => {
            ...
            deployPollRef.current = setInterval(async () => {
                try {
                    ...
                    if (!data.has_deployment) {
                        console.log(
                            "🔍 Deployment completed - no active deployment"
                        );
                        ...
                        if (data.last_deployment_url) {
                            const url = String(
                                ...
                                ? data.last_deployment_url
                                : `https://${data.last_deployment_url}`;
                    ...
                    if (isReady && data.deployment_url) {
                        const url = String(data.deployment_url).startsWith(
                            "http"
                        )
                            ? data.deployment_url
    ...
    async function start() {
        try {
            ...
            const r = await fetch(
                `${API_BASE}/api/projects/${projectId}/preview/start`,
                { method: "POST" }
            );
            if (!r.ok) {
    ...
    async function loadTree(dir = ".") {
        try {
            const r = await fetch(
                `${API_BASE}/api/repo/${projectId}/tree?dir=${encodeURIComponent(
                    dir
                )}`
            );
            const data = await r.json();
            ...
            if (Array.isArray(data)) {
                ...
                for (const entry of data) {
                    if (entry.type === "dir") {
                        try {
                            const subContents = await loadSubdirectory(
                                entry.path
                            );
                            newFolderContents.set(entry.path, subContents);
                        } catch (err) {
                            console.error(
                                `Failed to load contents for ${entry.path}:`,
                                err
                            );
                        }
                    }
                ...
                if (entry.type === "dir") {
                    ...
                    if (!newMap.has(fullPath)) {
                        loadSubdirectory(fullPath).then((subContents) => {
                            setFolderContents((prev2) =>
                                new Map(prev2).set(fullPath, subContents)
                            );
                        });
                    }
                }
    ...
    async function openFile(path: string) {
        try {
            const r = await fetch(
                `${API_BASE}/api/repo/${projectId}/file?path=${encodeURIComponent(
                    path
                )}`
            );
    ...
    async function loadSettings(projectSettings?: {
        cli?: string;
        model?: string;
    }) {
        try {
            console.log(
                "🔧 loadSettings called with project settings:",
                projectSettings
            );
            ...
            if (!hasCliSet || !hasModelSet) {
                ...
                if (globalResponse.ok) {
                    ...
                    if (!hasModelSet) {
                        const cliSettings =
                            globalSettings.cli_settings?.[
                                hasCliSet || defaultCli
                            ];
        ...
        try {
            ...
            if (r.ok) {
                ...
                if (project.selected_model) {
                    console.log(
                        "✅ Setting model from project:",
                        project.selected_model
                    );
                    const normalizedProjectModel = normalizeModelId(
                        project.preferred_cli || preferredCli,
                        project.selected_model
                    );
                ...
                const followGlobal =
                    !project.preferred_cli && !project.selected_model;
                ...
                if (project.initial_prompt) {
                    ...
                    localStorage.setItem(
                        `project_${projectId}_hasInitialPrompt`,
                        "true"
                    );
                ...
                const initialPromptFromUrl =
                    searchParams?.get("initial_prompt");
    ...
    async function runAct(messageOverride?: string, externalImages?: any[]) {
        ...
        try {
            ...
            const r = await fetch(
                `${API_BASE}/api/chat/${projectId}/${endpoint}`,
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
    ...
    const startDependencyInstallation = async () => {
        try {
            const response = await fetch(
                `${API_BASE}/api/projects/${projectId}/install-dependencies`,
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                }
            ...
            if (response.ok) {
                ...
                console.warn(
                    "⚠️ Failed to start dependency installation:",
                    errorText
                );
            }
        } catch (error) {
    ...
    const handleRetryInitialization = async () => {
        ...
        try {
            const response = await fetch(
                `${API_BASE}/api/projects/${projectId}/retry-initialization`,
                {
                    method: "POST",
                }
            );
            ...
            setInitializationMessage(
                "Failed to retry initialization. Please try again."
            );
        }
    };
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && projectId) {
            const storedHasInitialPrompt = localStorage.getItem(
                `project_${projectId}_hasInitialPrompt`
            );
            const storedTaskComplete = localStorage.getItem(
                `project_${projectId}_taskComplete`
            );
    ...
    useEffect(() => {
        if (!hasActiveRequests && !previewUrl && !isStartingPreview) {
            if (!previousActiveState.current) {
                ...
                console.log(
                    "✅ Task completed, ensuring preview server is running"
                );
            }
            start();
        }
    ...
    useEffect(() => {
        ...
        const handleBeforeUnload = () => {
            ...
            navigator.sendBeacon(
                `${API_BASE}/api/projects/${projectId}/preview/stop`
            );
        };
                ...
                <div className="h-full w-full flex">
                    ...
                    >
                        ...
                        <div className="p-4 rounded-bl-2xl">
                            <ChatInput
                                ...
                                placeholder={
                                    mode === "act"
                                        ? "Ask Claudecode desktop..."
                                        : "Chat with Claudecode desktop..."
                                }
                                mode={mode}
                    ...
                    >
                        ...
                        <div className="flex-1 min-h-0 flex flex-col">
                                                            ...
                                                            onClick={async () => {
                                                                ...
                                                                try {
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        console.log(
                                                                            "🚀 Deployment started, polling for status..."
                                                                        );
                                                                        ...
                                                                        if (
                                                                            ...
                                                                            const url =
                                                                                data.deployment_url.startsWith(
                                                                                    "http"
                                                                                )
                                                                                    ? data.deployment_url
                                                                                    : `https://${data.deployment_url}`;
                                                                    ...
                                                                    alert(
                                                                        "Publish failed. Check Settings and tokens."
                                                                    );
                                                                    setDeploymentStatus(
                                                                        "idle"
                                                                    );
                            ...
                            ${
                                assistantBrandColors[preferredCli] ||
                                assistantBrandColors.claude
                            }66 0%, 
                            ${
                                assistantBrandColors[preferredCli] ||
                                assistantBrandColors.claude
                                                            ...
                                                            >
                                                                ...
                                                                <div className="w-40 h-40 mx-auto mb-6 relative">
                                                                    <div
                                                                        ...
                                                                        style={{
                                                                            backgroundColor:
                                                                                assistantBrandColors[
                                                                                    preferredCli
                                                                                ] ||
                                                                                assistantBrandColors.claude,
                                                                            mask: "url(/Symbol_white.png) no-repeat center/contain",
                                                                            WebkitMask:
                                                                                "url(/Symbol_white.png) no-repeat center/contain",
                                                                            opacity: 0.9,
                                                                        }}
                                                                    />
                                            ...
                                            <div className="flex-1 flex flex-col bg-white dark:bg-[#0d0d0d] min-w-0">
                                                {selectedFile ? (
                                                        ...
                                                        <div className="flex-1 overflow-hidden">
                                                            <div className="w-full h-full flex bg-white dark:bg-[#0d0d0d] overflow-hidden">
                                                                ...
                                                                <div className="flex-1 overflow-auto custom-scrollbar">
                                                                    ...
                                                                    >
                                                                        <code
                                                                            className={`language-${getFileLanguage(
                                                                                selectedFile
                                                                            )}`}
                                                    ...
                                                    <div className="flex-1 flex items-center justify-center bg-white dark:bg-[#0d0d0d]">
                                                        <div className="text-center">
                                                            <span className="w-16 h-16 mb-4 opacity-10 text-gray-400 dark:text-[#3c3c3c] mx-auto flex items-center justify-center">
                                                                <FaCode
                                                                    size={64}
                                                                />
                                                            </span>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-600 dark:text-gray-400">
                                        Deploy with Vercel, linked to your
                                        GitHub repo
                                    </p>
                                </div>
                            </div>
